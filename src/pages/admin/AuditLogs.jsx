        ...
        <AdminLayout>
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-gray-500 text-sm">Complete history of all system actions</p>
                </div>
                    ...
                    <table className="table table-sm w-full">
                        <thead className="bg-gray-50 border-b border-gray-200">
                            <tr>
                                ...
                                <th className="text-xs font-bold text-gray-500 uppercase">Action</th>
                                <th className="text-xs font-bold text-gray-500 uppercase">Description</th>
                                <th className="text-xs font-bold text-gray-500 uppercase">Date & Time</th>
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
                                ...
                                <tr key={log.id} className="hover:bg-gray-50 border-b border-gray-100 last:border-0">
                                    ...
                                    <td>
                                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${log.profiles?.role === 'admin' ? 'bg-black text-white' : 'bg-gray-100 text-gray-500'}`}>
                                            {log.profiles?.role || '—'}
                                        </span>
                                    </td>
                                    <td>
                                        <span className={`text-xs font-bold px-2 py-1 rounded-full ${actionColors[log.action] || 'bg-gray-100 text-gray-600'}`}>
                                            {log.action?.replace('_', ' ').toUpperCase()}
                                        </span>
                                    </td>
                                    <td className="text-sm text-gray-600 max-w-xs">{log.description}</td>
